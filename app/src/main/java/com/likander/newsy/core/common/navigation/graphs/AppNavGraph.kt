...
fun AppNavGraph(
    ...
    NavHost(
        ...
        composable(route = AppDestinations.ErrorScreen.route) {
            ErrorContent(
                buttonName = stringResource(id = R.string.go_back),
        ...
        composable(
            ...
            val articleId = it.arguments?.getString(ARTICLE_ID)
            val screenType = it.arguments?.getString(SCREEN_TYPE)
...
private class AppNavGraphActions(navController: NavController) {
    ...
    val navigateToHomeScreen: () -> Unit = {
        navController.navigateSingleTopTo(AppDestinations.HomeScreen.route)
    }

    val navigateToDetailsScreen: (id: Int, screenType: String) -> Unit = { id, screenType ->
        navController.navigateSingleTopTo("${AppDestinations.DetailsScreen.route}/$id&$screenType")
    }

    val navigateToHeadlineScreen: () -> Unit = {
        navController.navigateSingleTopTo(AppDestinations.HeadlineScreen.route)
    }

    val navigateToDiscoverScreen: () -> Unit = {
        navController.navigateSingleTopTo(AppDestinations.DiscoverScreen.route)
    }

    val navigateToAccountGraph: () -> Unit = {
        navController.navigateSingleTopTo(AppDestinations.AccountGraph.route)
    }

    val navigateToSearchScreen: () -> Unit = {
        navController.navigateSingleTopTo(AppDestinations.SearchScreen.route)
    }
}